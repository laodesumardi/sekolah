        ...
        <div class="max-w-7xl mx-auto">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                <div class="mb-4 sm:mb-0">
                    <h1 class="text-2xl sm:text-3xl font-bold">Edit Sosial Media</h1>
                    <p class="text-primary-100 mt-2 text-sm sm:text-base">Edit informasi platform sosial media</p>
                </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            ...
            <div class="bg-gray-50 px-6 py-4 border-b border-gray-200">
                <h2 class="text-lg font-semibold text-gray-900">Form Edit Sosial Media</h2>
                <p class="text-sm text-gray-600 mt-1">Edit informasi platform sosial media: {{ $social_medium->name }}</p>
            </div>
            ...
            <div class="p-6">
                <form action="{{ route('admin.social-media.update', $social_medium) }}" method="POST" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select id="name" 
                                ...
                                <option value="facebook" {{ old('name', $social_medium->name) === 'facebook' ? 'selected' : '' }}>Facebook</option>
                                <option value="instagram" {{ old('name', $social_medium->name) === 'instagram' ? 'selected' : '' }}>Instagram</option>
                                <option value="youtube" {{ old('name', $social_medium->name) === 'youtube' ? 'selected' : '' }}>YouTube</option>
                                <option value="twitter" {{ old('name', $social_medium->name) === 'twitter' ? 'selected' : '' }}>Twitter</option>
                                <option value="tiktok" {{ old('name', $social_medium->name) === 'tiktok' ? 'selected' : '' }}>TikTok</option>
                                <option value="whatsapp" {{ old('name', $social_medium->name) === 'whatsapp' ? 'selected' : '' }}>WhatsApp</option>
                                <option value="linkedin" {{ old('name', $social_medium->name) === 'linkedin' ? 'selected' : '' }}>LinkedIn</option>
                            </select>
                            @error('name')
                        ...
                        <div>
                            ...
                            <div class="flex items-center space-x-3">
                                <input type="color" 
                                       id="color" 
                                       name="color" 
                                       value="{{ old('color', $social_medium->color) }}"
                    ...
                    <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                        ...
                        <div class="flex items-center space-x-3">
                            <div id="preview-icon" class="h-10 w-10 rounded-full flex items-center justify-center text-white" style="background-color: {{ $social_medium->color }};">
                                {!! $social_medium->icon_html !!}
                            </div>
                            <div>
                                <p id="preview-name" class="text-sm font-medium text-gray-900">{{ $social_medium->name }}</p>
                                <p id="preview-url" class="text-xs text-gray-500">{{ $social_medium->url }}</p>
                            </div>
                        </div>
                    </div>
